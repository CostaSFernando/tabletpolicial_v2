...
$.post('http://tabletpolicial_v2/GerarCrimes', JSON.stringify({}), (data) => {
    const nameList = data.crimes.sort((a, b) => (a.name2 > b.name2) ? -1 : 1);
    $(".scroll-crimes").html(`
    <div class="search-crime">
            <input type="text" placeholder="Pesquisar pelo crime..." class="">
            <div class="meses">
...
`);
    ...
    function SomarCheckbox() {
        ...
        meses = total
        $.post('http://tabletpolicial_v2/GerarConfig', JSON.stringify({}), (data) => {
        ...
        titles.forEach((title) => {
            ...
            $(document).on("click", ".buttonprender", () => {
                ...
                if ($('.prender-s').val() == "") {
                    ...
                    let descript = $('.textarea').val()
                    $.post('http://tabletpolicial_v2/Prender', JSON.stringify({ user, servicos, descript }), (data) => {
