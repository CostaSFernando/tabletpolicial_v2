...
$.post('http://tabletpolicial_v2/GerarMultas', JSON.stringify({}), (data) => {
    const nameList = data.multas.sort((a, b) => (a.name2 > b.name2) ? -1 : 1);
    $(".scroll-multas").html(`
    <div class="search-multa">
            <input type="text" placeholder="Pesquisar pela multa..." class="">
            <div class="multa">
...
${nameList.map((item) => (`
    <div class="multinha">
        <input type="checkbox" id="${item.nome}" value="${item.multa}" class="multacheckbox">
            ...
            <div class="titulo">
            <h1>${item.titulo ? item.titulo : 'Titulo Idefinido.'} <img src="./imagens/svg/icon-multa.svg" alt=""><span>R$${item.multa}</span></h1>
...
`);
    ...
    function SomarCheckbox() {
        ...
        multa = total
        $.post('http://tabletpolicial_v2/GerarConfigMulta', JSON.stringify({}), (data) => {
    ...
    $(document).on("click", ".buttonmultar", () => {
        ...
        if ($('.multar-s').val() == "") {
            ...
            $.post('http://tabletpolicial_v2/Multar', JSON.stringify({ user, valor, desc }), (data) => {
...
$(document).on("click", ".opcoesdemulta", () => {
    if ($("#wrapper-multar").is(":visible")) {
        ...
        $('.opcoesdemulta').html(`Opções de prisão`)
    } else {
        $('#wrapper-multar').fadeIn(200)
        $('.opcoesdemulta').addClass('open')
        $('.opcoesdemulta').html(`Fechar`)
    }
})